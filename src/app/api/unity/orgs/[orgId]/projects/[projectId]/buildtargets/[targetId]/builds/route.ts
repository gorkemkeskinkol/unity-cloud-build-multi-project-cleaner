...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ orgId: string; projectId: string; targetId: string }> }
) {
  try {
    ...
    const { orgId, projectId, targetId } = await params;
    ...
    const unityUrl = new URL(`${BASE_URL}/orgs/${orgId}/projects/${projectId}/buildtargets/${encodedTargetId}/builds`);
    ...
    console.error('Unity target builds API error:', error);
    return NextResponse.json(
